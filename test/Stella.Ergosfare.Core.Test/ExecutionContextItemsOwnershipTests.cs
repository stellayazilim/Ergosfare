...
/// <see cref="ErgosfareContext"/>: a dictionary adopted from the caller (a
...
public class ExecutionContextItemsOwnershipTests
{
    ...
    public void Return_ShouldDetachAdoptedDictionary_WithoutClearingIt()
    {
        ...
        var context = ErgosfareContextPool.Rent(callerItems, CancellationToken.None);
    ...
    public void Rent_AfterAdoptedDispatch_ShouldNotExposePreviousCallersItems()
    {
        ...
        var first = ErgosfareContextPool.Rent(callerItems, CancellationToken.None);
        ...
        var second = ErgosfareContextPool.Rent(items: null, CancellationToken.None);
    ...
    {
        var first = ErgosfareContextPool.Rent(items: null, CancellationToken.None);
    ...
    public void Reset_WithNewAdoptedDictionary_ShouldReplaceDetachedOne()
    {
        ...
        var context = ErgosfareContextPool.Rent(firstCaller, CancellationToken.None);
        ErgosfareContextPool.Return(context);

        var reused = ErgosfareContextPool.Rent(secondCaller, CancellationToken.None);

        Assert.False(reused.Has("a"));
        Assert.True(reused.Has("b"));

        reused.Set("c", 3);
        ErgosfareContextPool.Return(reused);
