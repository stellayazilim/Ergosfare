...
public class VoidPipelineBodyTMessageTests:
    ...
    IClassFixture<MessageDependencyFixture>
{
    ...
    public VoidPipelineBodyTMessageTests(
    ...
    public async Task SingleAsyncHandlerShouldInvokeSingleAsyncHandlerWithoutResponse()
    {
       ...
       var nonExceptionAsync = await Record.ExceptionAsync( async () =>
           await VoidPipelineBody<StubMessage>.Run(
               message, dependencies, null, null, _executionContextFixture.Ctx, _messageDependencyFixture.ServiceProvider));
    ...
    [Fact]
    public async Task Run_ShouldAssignInvokedResult_WhenHandlerThrowsException()
    {
        // Arrange
        var message = new StubMessage();
