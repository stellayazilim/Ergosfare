...
/// dispatch generics rooted through <c>GeneratedPlanRegistry</c> — message roots plus
...
public class PlanRegistryTests
{
    ...
    public void Emission_RootsDispatchableMessagesOnly()
    {
        ...
        Assert.DoesNotContain("GeneratedPlanRegistry.AddMessage<global::TestApp.VoidPing>();", source);
        Assert.DoesNotContain("GeneratedPlanRegistry.AddMessage<global::TestApp.MetricRecorded>();", source);
        Assert.DoesNotContain("GeneratedPlanRegistry.AddResult<global::TestApp.TypedPing, string>();", source);
        Assert.DoesNotContain("GeneratedPlanRegistry.AddResult<global::TestApp.TimeQuery, global::System.DateTimeOffset>();", source);
        Assert.DoesNotContain("GeneratedPlanRegistry.AddStream<global::TestApp.NumberStream, int>();", source);
    ...
    {
        var result = GeneratorTestHost.RunWithAllCandidates(HiddenSource);
        ...
        // publish.
        Assert.DoesNotContain("GeneratedPlanRegistry.AddMessage<global::TestApp.HiddenTyped>();", source);
        Assert.DoesNotContain("GeneratedPlanRegistry.AddResult<global::TestApp.HiddenTyped, string>();", source);
        Assert.DoesNotContain("GeneratedPlanRegistry.AddStream<global::TestApp.HiddenStream, int>();", source);
    }

    [Fact]
    public void ExecutingRegisterAll_PopulatesTheProcessWideRootStore()
    {
        var result = GeneratorTestHost.RunWithAllCandidates(Source, referenceModuleBuilders: false);
        ...
        Assert.IsAssignableFrom<Stella.Ergosfare.Core.Abstractions.IPipelineExecutor>(GeneratedPlanRegistry.FindStagedVoidPlan(voidPing));
    }
}
