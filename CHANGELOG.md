...
* **`MessageRegistry` registration is now serialized, and readers are lock-free.** `Register` and `RegisterDescriptors` run under a single gate (with a lock-free fast path for already-processed types), fixing two real races: concurrent registrations corrupting the backing lists/index (including the check-then-act on the processed-types set, which could yield duplicate descriptors for the same handler type), and dispatch-time registry enumeration (the resolve strategy's assignability scan) throwing "collection was modified" while another thread registered. Realistic triggers: the registry is a process-wide singleton shared by parallel test hosts, `IMessageRegistry.Register` is public runtime API, and `RegisterPlainMessagesOnSpot` registers at dispatch time.
* **Immutable snapshots for readers.** Enumeration and `Count` read a `volatile` descriptor-array snapshot republished at the end of every effective registration; `MessageDescriptor` stage collections are copy-on-write arrays swapped wholesale on mutation. A reader building a pipeline shape mid-registration observes either the previous or the new stage — never a torn list. Publication order (stage swaps → version bump → snapshot swap) guarantees a reader that observes the new `Version` also observes the mutations behind it. Dispatch pays nothing: no locks, no allocation changes on the hot path.
* Registration semantics are unchanged: same descriptors, same linking, same idempotence — now also under concurrency (covered by new stress tests).
* **Resolve-strategy cache poisoning fixed** (caught by the new stress tests): `ActualTypeOrFirstAssignableTypeMessageResolveStrategy` invalidated its lookup cache by clearing it whenever the registry count changed. A lookup computed against a pre-registration snapshot could insert its (negative) result *after* the clear, and with the count check already caught up, the poisoned entry was never invalidated again — `Find` then returned `null` forever for a registered type. The cache now stamps every entry with the registry size it was computed against: exact-match entries stay valid forever (descriptors are mutated in place, never replaced), negative and assignable-fallback entries self-invalidate as soon as the registry grows. No global clears, no cross-thread ordering sensitivity.
* **Hot-path LINQ removed.** Event broadcast filtered handlers with `Where(...).ToList()` on every publish (iterator + closure + list per dispatch); it now filters with a plain loop that allocates nothing when the predicate accepts every handler (the default) and materializes a list only from the first rejection onward. Pipeline-shape building dropped its `OrderBy/ThenBy/Where/Intersect/Select/ToArray` chain (the `Intersect` allocated a hash set per descriptor) for a filter loop plus a single in-place sort.
* Also fixed in passing: `AsyncBroadcastMediationStrategy` dropped its unused `resultAdapterService` constructor parameter (leftover of the typed-interceptor severance; events never adapt results).
