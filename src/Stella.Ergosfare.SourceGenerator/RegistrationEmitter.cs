...
internal static class RegistrationEmitter
{
        ...
        bool hasKeyedServiceExtensions)
    {
        ...
        // Messages hidden from discovery that a registration nevertheless reaches. They are
        // rooted, not registered: the root only lets a dispatch close its generic without
        // MakeGenericType, and which rows run is still whatever the container selected.
    ...
    /// </summary>
    /// <summary>
    ///     One message's dispatch roots: the message generic, then one per result contract it
    ///     declares. Both closed at compile time, which is what keeps <c>MakeGenericType</c>
